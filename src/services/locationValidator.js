...
class LocationValidator {
    constructor() {
        ...
        this.edoMexMunicipalities = [
            'atizapan de zaragoza', 'coacalco', 'cuautitlan', 'cuautitlan izcalli', 'chalco',
            'chicoloapan', 'chimalhuacan', 'ecatepec', 'huixquilucan', 'ixtapaluca',
            'la paz', 'naucalpan', 'nezahualcoyotl', 'nicolas romero', 'tecamac', 'tepotzotlan',
            'texcoco', 'tlalnepantla', 'tultitlan', 'valle de chalco', 'zumpango'
        ];

        // Municipios de Hidalgo cercanos al área metropolitana
        this.hidalgoMunicipalities = [
            'tizayuca', 'mineral de la reforma', 'pachuca'
        ];

        // Otras zonas aceptables mencionadas en el prompt
        this.otherAcceptableAreas = [
            'satelite', 'santa fe', 'polanco', 'reforma', 'zona rosa', 'condesa', 'roma',
            'del valle', 'doctores', 'centro', 'centro historico', 'insurgentes',
            'perisur', 'coyoacan', 'xochimilco', 'tlalpan'
        ];
    ...
    isValidLocation(location) {
        ...
        return this.validAreas.some(area => {
            ...
            return cleanLocation.includes(normalizedArea) || normalizedArea.includes(cleanLocation);
        });
    }
    ...
    extractPossibleLocations(text) {
        ...
        const knownInvalidAreas = [
            'guadalajara', 'monterrey', 'queretaro', 'puebla', 'tijuana', 'cancun',
            'veracruz', 'merida', 'toluca', 'leon', 'aguascalientes', 'morelia',
            'chihuahua', 'saltillo', 'hermosillo', 'culiacan', 'mazatlan',
            'torreon', 'durango', 'tampico', 'reynosa', 'matamoros', 'nuevo laredo',
            'acapulco', 'oaxaca', 'tuxtla', 'villahermosa', 'campeche', 'chetumal'
        ];
    ...
    validateMessageLocation(message) {
        const { validLocations, invalidLocations } = this.extractPossibleLocations(message);
